...
class ALS(object):
    ...
    def _users_mul_ratings(self, users, ratings_T):
        """将密集矩阵（用户矩阵）与稀疏矩阵（评级矩阵）相乘。
        结果（项）是k * n矩阵，n代表item_id的数量.
        Arguments:
    ...
    def _items_mul_ratings(self, items, ratings):
        """将密集矩阵（项矩阵）与稀疏矩阵（等级矩阵）相乘。
        结果（用户）是k * m矩阵，m代表user_id的数量.
        Arguments:
...
def format_prediction(item_id, score):
    return "movie_id:%d score:%.2f" % (item_id, score)
...
def main():
    ...
    for user_id, prediction in zip(user_ids, predictions):
        ...
        print("User id:%d  推荐: %s" % (user_id, _prediction))
